...
def llm_processing(processed_doc):
    ...
    try:
        ...
        processed_doc["review_required"] = True
        processed_doc["llm_score"] = 0.0
        processed_doc["raw_text_for_llm"] = ""
        processed_doc["rule_based_text"] = ""
        return processed_doc
...
def process_monitoring_doc(processed_doc, number_of_items):
    try:
        ...
        logging.error(f"Error processing monitoring document: {e}")
        return None
...
def flattened_doc(processed_doc):
    ...
    try:
        ...
        logging.error(f"Error flattening document: {e}")
